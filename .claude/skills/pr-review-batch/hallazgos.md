...
**Los dos skills de PR traen este archivo adentro, idéntico.** Lo lee `pr-review` en su Paso 5, y
`pr-review-batch` le pasa **su propia copia** por ruta a cada uno de sus N agentes. Está afuera del
`SKILL.md` y no inlineado porque así no le cuesta contexto al padre del batch, que no busca
hallazgos, ni hay que repetir cien líneas en cada prompt. Que las dos copias no se separen lo
verifica `test_copias_de_skills.py`, que da rojo ante un byte de diferencia.
...
[`sin-deuda.md`](sin-deuda.md) y no se repite acá; lo que sigue es cómo aterriza
sobre un diff.
