...
# sobre el sentido de la frase.** Medido sobre el PR #6 el 2026-08-29: **375 líneas candidatas
# sin el filtro contra 112 con él**, o sea 263 que eran `Paso N`, `#N`, `T0NN` y encabezados
# numerados. Un bloque donde el 70 % es ruido no se lee, y un bloque que no se lee es un eje
# apagado.
#
...
def main() -> None:
    ...
    # Un solo recorrido del diff, y de acá salen los tres contadores de ejes **y** el bloque de
    # afirmaciones numéricas. Cada línea agregada viaja con el archivo al que pertenece: sin eso
    # no se puede distinguir un `#` de comentario de un `#` de encabezado markdown.
    agregadas_por_archivo: list[tuple[str, str]] = []
    archivo = ""
    for linea in diff.splitlines():
        if linea.startswith("+++ b/"):
            archivo = linea[6:]
        elif linea.startswith("+") and not linea.startswith("+++"):
            agregadas_por_archivo.append((archivo, linea[1:]))
    agregadas = [cuerpo for _, cuerpo in agregadas_por_archivo]
    texto_agregado = "\n".join(agregadas)
    ...
    # `def ` va con los cuatro de GDScript porque **el harness de este repo es Python** y la
    # convención de tipado aplica igual: sin él, un PR que sólo toca `.claude/scripts/` mide 0 y
    # apaga el eje. Medido sobre el PR #6 el 2026-08-29: 0 firmas sin `def ` en la lista, 18 con
    # ella — o sea que el eje de tipos venía apagado justo en el PR que agrega cinco scripts.
    tipos = len(re.findall(r"^\s*(class_name|enum |signal |func |def )", texto_agregado, re.M))
    # Sólo sobre lo que NO es prosa: en un `.md`, `^#` es un encabezado y no un comentario.
    # Contarlos inflaba el número sin mover nunca la decisión —`len(docs) >= 1` ya la toma—, o
    # sea que era una medición falsa impresa al lado de una correcta.
    coment = len(
        [c for a, c in agregadas_por_archivo if COMENTARIO.match(c) and not PROSA.search(a)]
    )
    ...
    for archivo, cuerpo in agregadas_por_archivo:
