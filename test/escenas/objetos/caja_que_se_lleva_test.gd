...
## Dónde se para el jugador para levantar una caja del estante, relativo a ella. Al costado y
## no de frente: el estante del depósito está contra la pared y sólo se lo alcanza de lado.
const AL_LADO_DEL_ESTANTE := Vector3(1.0, -1.8, 0.0)
...
func test_la_caja_se_lleva_de_frente_y_sin_tapar_la_mira() -> void:  # 047-AC6
	var almacen: Node3D = await _almacen_con_jugador_quieto()
	var jugador: Node3D = almacen.get("_jugador")
	...
	_comprobar_la_mira_libre(jugador, caja, "en la mano")


func test_la_caja_se_saca_del_estante_y_se_vuelve_a_apoyar() -> void:  # 047-AC11
	# Las dos mitades del mismo gesto. Las dos se rompen si el cuerpo del jugador crece con la
	# caja en la mano: agarrarla pegado al estante lo empuja, y después no llega a dejarla.
	var almacen: Node3D = await _almacen_con_jugador_quieto()
	var jugador: CharacterBody3D = almacen.get("_jugador")
	var caja: Node3D = almacen.get("_cajas_de_productos")[Producto.Id.ACTRONCITO]
	var origen := caja.global_position
	jugador.global_position = origen + AL_LADO_DEL_ESTANTE
	jugador.rotation.y = PI / 2.0
	var antes := jugador.global_position
	_accion(jugador, caja, ReglasDeLosObjetos.ACCION_AGARRAR)
	for cuadro in 10:
		await get_tree().physics_frame
	(
		assert_vector(jugador.global_position)
		. override_failure_message("agarrar la caja empujó al jugador")
		. is_equal_approx(antes, Vector3.ONE * 0.02)
	)
	_accion(jugador, null, ReglasDeLosObjetos.ACCION_AGARRAR)
	(
		assert_float(caja.global_position.y)
		. override_failure_message(
			"la caja no volvió al estante: quedó en %v" % caja.global_position
		)
		. is_equal_approx(origen.y, 0.01)
	)
