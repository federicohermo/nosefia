...
Copia ?ntegra de `.claude/rules/dominio.md`.
...
**La pureza la verifica `gate_de_capas.py`**, y esa tabla es de dónde sale su lista. El `extends`
va por **lista blanca** —`RefCounted`, `Resource`, o un `class_name` del propio dominio—, así que
...
Y hay que decir hasta dónde llega, que es la mitad honesta: **mira los patrones de esa tabla y
nada más, y dos de sus filas sólo en parte.** De los callbacks busca los tres que la fila nombra
—`_process`, `_physics_process`, `_input`—, no los demás de `Node`: un `_ready` acá pasa en
verde. Y los «nodos de UI» los caza cuando se los **extiende** —ahí entra la lista blanca— o
cuando son un `class_name` de `ui/` —ahí entra la dirección—, pero un `Label` nombrado como tipo
pasa. No es un linter de GDScript, y agregarle el siguiente es una decisión, no un trámite — el
criterio para pedirlo es el del párrafo de arriba: ¿esto obliga a levantar una escena para
probarlo?
