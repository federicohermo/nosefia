## Una de las tareas obligatorias del turno: qué tipo es y si ya se hizo.
##
...
## no cambia nada, y es de ese `false` que se agarra `Turno` para no contarla dos veces sin
## llevar un registro propio de cuáles ya cumplió.
func completar() -> bool:
	if _completada:
		return false
	_completada = true
