...
##
## Un `String` es el modo de falla que nombra `CLAUDE.md`: `"actronsito"` no rompe nada, el `if`
## simplemente no entra nunca. Y la instancia tampoco sirve como identidad: `Catalogo.de()`
## construye un producto nuevo en cada llamada, así que dos Actroncitos son objetos distintos y un
